...
- 現在の申請状態: 審査準備完了（提出物の下書きにiOSアプリ `1.0 (34)` と広告削除App内課金を追加済み。Sandbox／TestFlight購入・復元確認とユーザー最終確認が終わるまで審査提出を停止）
...
- [x] 2026-09-02受領のiPhone掲載用スクリーンショット4枚へ差し替え、順序と表示を確認した
- [x] iPad 13インチの掲載用スクリーンショットを登録し、ファイル名・表示・登録件数を再読み込み後に確認した
...
- [x] 非消費型App内課金 `jp.khunryo.forgetcheck.removeads` を作成し、日本100円、配信地域、5言語、審査メモ、審査画像を登録した
- [ ] Sandbox／TestFlightで購入成功、キャンセル、保留、復元、再起動後の広告非表示を確認した
- [x] iOSアプリ `1.0 (34)` と広告削除商品を同一の「提出物の下書き」へ追加した
- [x] 審査連絡先、広告・プライバシー文言、価格・配信地域、ローカライズをApp Store Connectで保存・表示確認した
- [x] 「審査用に追加」後、提出物の下書きに2項目（iOSアプリと広告削除App内課金）が含まれることを確認した
- [ ] ユーザーから掲載画像と審査送信の最終承認を受けた
...
- App Store Connect: Codemagicの配信処理とApple側の処理が完了。ビルド34をバージョン1.0へ選択済みで、TestFlight上は「承認済み」。購入・復元の実機確認は未完了
...
|---|---|---|---|
| `01-category-selection-20260902.png` | `C:\dev\fishing\forget-check\store-assets\2026-09-02\01-category-selection-20260902.png` | 種類選択画面 | 1242×2688 RGB PNG・登録済み・表示確認済み（1枚目） |
| `02-check-progress-20260902.png` | `C:\dev\fishing\forget-check\store-assets\2026-09-02\02-check-progress-20260902.png` | 3 / 6件チェック済みの進捗画面 | 1242×2688 RGB PNG・登録済み・表示確認済み（2枚目） |
| `03-template-overview-20260902.png` | `C:\dev\fishing\forget-check\store-assets\2026-09-02\03-template-overview-20260902.png` | テンプレート作成・種類選択 | 1242×2688 RGB PNG・登録済み・表示確認済み（3枚目） |
| `04-template-items-20260902.png` | `C:\dev\fishing\forget-check\store-assets\2026-09-02\04-template-items-20260902.png` | アイコン選択・持ち物編集 | 1242×2688 RGB PNG・登録済み・表示確認済み（4枚目） |
...
現在のビルドはiPhoneとiPadの両方を対象（`TARGETED_DEVICE_FAMILY = "1,2"`）としている。iPhone画像は日本語UI、iPad画像は英語UIの実画面を登録済み。英語ローカライゼーションの品質をさらに揃える場合は、英語UIのiPhone画像も将来追加する。

## 2026-09-02 13インチiPad掲載用スクリーンショット

登録場所: App Store Connect → Forget Check → 配信 → iOSアプリ 1.0 → プレビューとスクリーンショット → iPad → 13インチディスプレイ

- ファイル名: `ForgetCheck_AppStore_Screenshot_iPad13_01_20260902.png`
- ローカル絶対パス: `C:\dev\fishing\forget-check\store-assets\2026-09-02\ForgetCheck_AppStore_Screenshot_iPad13_01_20260902.png`
- 仕様: 2064×2752、RGB PNG、透過なし
- SHA-256: `3FE0E10190426E2E8217849A60218AC46E17B023C2008567F923D740534362B3`
- 内容: 英語表示の種類選択画面、3種類の既定テンプレート、広告削除 `$0.99` と購入復元の導線
- 撮影環境: CodemagicのiPad Pro 13-inchシミュレーター。提出ビルド34と同じアプリソースを使用し、起動40秒後の実画面を採用
- Codemagicビルド: https://codemagic.io/app/6a7bcbd5cfca7ac6fad54354/build/6a9825600e99155928884c41
- 撮影タグ／コミット: `forget-check-ios-build-20260902-ipad-shot-5` / `b915ce1`
- 元画像: `C:\dev\fishing\forget-check\store-assets\2026-09-02\ipad-13-final\build\forget-check-ios\screenshots\ForgetCheck_iPad_13inch_40s.png`
- ダウンロード済み成果物: `C:\Users\81906\Downloads\tsuriba-map_10_artifacts.zip`（SHA-256 `797E45EF62A42F29FB043EE77F171FFE6D3D961C66B2EBCA2F216DDC26D072D0`）
- 登録結果: App Store Connectへ登録し、再読み込み後にファイル名と `スクリーンショット数：1 / 10` を確認済み
...
- App内課金Apple ID: `6807809694`
- 種類: 非消費型
- 価格: 米国 `$0.99`、日本 `￥100.00`
- 配信地域: EU27地域を除く148 / 175地域
- ローカリゼーション: 日本語、英語（アメリカ）、中国語（簡体字）、韓国語、スペイン語（スペイン）
...
- 状態: ローカル実装の購入導線を表示確認済み。App Store Connectへ登録し、画像表示と保存済み状態を確認済み
- 提出物: App Store Connectの「提出物の下書き」へ追加済み。iOSアプリ `1.0 (34)` も同じ提出物へ追加済み
...
- プライバシーポリシーURL: `https://forget-check-jp.khunryo.chatgpt.site/privacy.html`（英語、日本語、中国語（簡体字）、韓国語、スペイン語の5言語へ登録済み）
- プライバシーポリシー修正: AdMobがIPアドレスからおおよその地域を推定する可能性を、日本語・英語で正確に追記。ローカルビルドと配信アーカイブを検証済み
- 公開サイト: 広告削除App内課金を追記したSitesバージョン15（ソース `706329aef69f51944fb402fa036ff913c827e8d1`）を本番公開済み。URLは `https://forget-check-jp.khunryo.chatgpt.site/`、デプロイIDは `appgdep_6a980ebf26248191b535fa1f0bdb2cc5`
...
- 審査連絡先: 氏名 `Ryohei Sugamata`、電話 `+81 90 6528 2236`、メール `tabitomo.support@gmail.com` を保存し、App Store Connect上で表示確認済み
- 審査メモ: 広告表示位置、端末内保存、ログイン不要、主要確認手順を英語で保存済み
- 審査説明用画像: `ForgetCheck_AppReview_Image_CoreFlow_20260830.png` を添付済み
- 13インチiPad画像: `ForgetCheck_AppStore_Screenshot_iPad13_01_20260902.png` を13インチディスプレイ欄へ登録し、表示と `1 / 10` を確認済み
- 審査動画: Appleから要求記録がないため、App Store Connectから外した状態を維持する
- アプリ価格: 無料を維持
- 広告削除App内課金: Apple ID `6807809694`、商品ID `jp.khunryo.forgetcheck.removeads`、非消費型。米国基準価格 `$0.99`、日本価格 `￥100.00`、EU27地域を除く148地域、5言語、審査メモ、審査画像を登録済み。提出物の下書きへ追加済み
- 提出候補ビルド: `1.0 (34)` をCodemagicからApp Store Connectへアップロードし、Apple処理完了後にバージョン1.0へ選択済み。TestFlight表示は「承認済み」
- 配信地域: EUを除外し、148地域で利用可能・27地域で利用不可を確認済み
- Mac（Apple Silicon）／Apple Vision Pro: 現在有効。未検証プラットフォームのため、iPhone／iPad限定にする場合は提出前に無効化する
- 外部登録操作: 上記の画像、掲載文、カテゴリ、配信権、年齢制限、プライバシー、審査メモ、審査連絡先、広告削除App内課金を保存済み。スペイン語・韓国語のサポートURLも登録済み。13インチiPad画像の登録後、App Store Connectの必須項目エラーが解消され、iOSアプリ `1.0 (34)` と広告削除App内課金の2項目を同じ提出物の下書きへ追加した。審査送信は未実施
...
- 申請日時: 未申請
- 提出バージョン／ビルド: `1.0 (34)` をアップロード・処理・選択済み。提出物の下書きへ追加済み
- 下書き内容: iOSアプリ `1.0 (34)`、非消費型App内課金 `Forget Check Remove Ads` の2項目
- 下書き最終更新: 2026-09-02 22:49 JST（App Store Connect表示）
- ユーザー最終確認日時: 未確認
- 現在のステータス: 審査準備完了（下書き、未送信）
- 審査結果・指摘: 未記録

## Claude引き継ぎ用記載

Forget CheckのApp Store Connectは https://appstoreconnect.apple.com/apps/6804138762/distribution/ios/version/inflight です。提出候補は入力拡大修正、Google AdMob、StoreKit 2の広告削除を含むバージョン `1.0 (34)` で、Apple処理完了・ビルド選択・TestFlight「承認済み」まで確認済みです。Sideloadly用IPAは `C:\Users\81906\Downloads\ForgetCheck-remove-ads-484432b-sideload.ipa`、App Store用IPAは `C:\dev\fishing\forget-check\artifacts\ForgetCheck-AppStore-484432b.ipa` です。iPhone 6.5インチ画像4枚は `C:\dev\fishing\forget-check\store-assets\2026-09-02` から登録・順序確認済みです。13インチiPad実画面はCodemagicビルド https://codemagic.io/app/6a7bcbd5cfca7ac6fad54354/build/6a9825600e99155928884c41 で撮影し、`C:\dev\fishing\forget-check\store-assets\2026-09-02\ForgetCheck_AppStore_Screenshot_iPad13_01_20260902.png`（2064×2752 RGB、SHA-256 `3FE0E10190426E2E8217849A60218AC46E17B023C2008567F923D740534362B3`）として13インチ欄へ登録、再読み込み後にファイル名と `1 / 10` を確認済みです。審査説明用画像 `C:\dev\fishing\forget-check\artifacts\app-review-images\ForgetCheck_AppReview_Image_CoreFlow_20260830.png` と、広告削除商品の審査画像 `C:\dev\fishing\forget-check\store-assets\2026-09-02\iap-review\ForgetCheck_IAP_Review_RemoveAds_20260902.png` も登録済みです。非消費型商品はApple ID `6807809694`、商品ID `jp.khunryo.forgetcheck.removeads`、米国 `$0.99`、日本 `￥100.00`、EU27を除く148地域、5言語です。公開サイトのSitesバージョン15は https://forget-check-jp.khunryo.chatgpt.site/ で本番公開済みです。Appleから動画要求はなく、審査用動画は添付しておらず、App Previewも未登録です。App Store Connectの必須項目エラーは解消され、提出物の下書きにはiOSアプリ `1.0 (34)` と `Forget Check Remove Ads` の2項目が入り、画面状態は「審査準備完了」です。未完了は、(1) Sandbox／TestFlightでの購入成功・キャンセル・保留・復元・再起動後の広告非表示を含む実機最終確認、(2) 掲載画像のユーザー最終確認、(3) 審査送信の明示承認です。最終の「審査へ提出」は未実施なので、ユーザーの明示承認なしに押さないでください。

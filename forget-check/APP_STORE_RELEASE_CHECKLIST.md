...
現在のビルドはiPhoneとiPadの両方を対象（`TARGETED_DEVICE_FAMILY = "1,2"`）としているため、13インチiPad用スクリーンショットも必要。これはWeb版の合成画像で代用せず、ビルド32をiPad実機またはiOSシミュレーターで起動して撮影する。現在登録中の英語（アメリカ）ローカライゼーションの画像は日本語UIなので、グローバル掲載品質を上げるには英語UIのiPhone／iPad実画面も別途撮影する。
...
## App Store Connect登録状況（2026-08-31）
...
- 公開サイト: Sitesバージョン14を本番デプロイ済み。公開URLとプライバシーURLはHTTP 200、日本語・英語のAdMob位置情報文言と更新日を確認済み
...
- アプリ内言語検査: 日本語を基準に、英語・中国語（簡体字）・韓国語・スペイン語の各49キーが一致し、欠落0・余分0を静的検査で確認済み
...
Forget CheckのApp Store Connectは https://appstoreconnect.apple.com/apps/6804138762/distribution/ios/version/inflight です。提出候補はバージョン1.0、ビルド32、状態は「提出準備中」です。iPhone 6.5インチ欄には `01-category-selection.png` → `04-check-progress.png` → `02-template-editor.png` → `03-splash-screen.png` の順で4枚登録済みです。不適切だった `forget-check-iphone-6.5.png` は削除済みです。審査説明用画像 `C:\dev\fishing\forget-check\artifacts\app-review-images\ForgetCheck_AppReview_Image_CoreFlow_20260830.png` はApp Review添付欄へ登録済み、審査メモも保存済みです。英語、日本語、中国語（簡体字）、韓国語、スペイン語の掲載文、カテゴリ「仕事効率化／旅行」、第三者コンテンツ配信権、年齢制限 `4+`、プライバシーポリシーURL、AdMobに対応する6種類のプライバシー回答は保存済みで、プライバシーラベルは2026-08-31に公開済みです。Appleから動画要求はなく、`forget-check-app-review-demo.mp4` はApp Store Connectから外したままです。未完了は、(1) `+81 90 6528 2236` と `tabitomo.support@gmail.com` の審査連絡先2欄をChromeで手入力して保存・再読み込み確認、(2) ビルド32の13インチiPad実画面スクリーンショット登録、(3) ビルド32の実機最終確認、(4) ユーザーの画像・掲載内容の最終確認、(5) 審査送信の別途承認です。現時点では審査へ提出しないでください。
